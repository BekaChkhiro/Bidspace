    ...
    const fetchTheatreCinemaAuctions = async (retryCount = 0) => {
      try {
        ...
        const theatreCinemaAuctions = data.filter(auction => 
          auction.meta?.auction_category === 'theatre_cinema'
        );
        setTheatreCinemaAuctions(theatreCinemaAuctions);
      } catch (error) {
        console.error('Error fetching theatre/cinema auctions:', error);
        setError('თეატრისა და კინოს აუქციონების ჩატვირთვისას მოხდა შეცდომა');
