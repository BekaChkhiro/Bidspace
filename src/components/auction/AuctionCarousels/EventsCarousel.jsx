...
const EventsCarousel = () => {
  const [eventsAuctions, setEventsAuctions] = useState([]);
  ...
  useEffect(() => {
    const fetchEventsAuctions = async (retryCount = 0) => {
      try {
        ...
        const eventsAuctions = data.filter(auction => 
          auction.meta.ticket_category === "ივენთები" && 
          auction.meta.visibility === true
        );

        if (eventsAuctions.length === 0) {
          setLoading(false);
          return;
        }

        setEventsAuctions(eventsAuctions);
        setError(null);
        ...
        if (retryCount < 2) {
          setTimeout(() => {
            fetchEventsAuctions(retryCount + 1);
          }, 2000);
        } else {
  ...
  if (loading || error || eventsAuctions.length === 0) {
    return null;
  }

  return (
    <div className="events-auctions-carousel">
