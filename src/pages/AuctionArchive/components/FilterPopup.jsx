...
const FilterPopup = ({ onClose, onApply, currentFilters }) => {
  ...
  // Update the hasActiveFilters function to be more precise
  const hasActiveFilters = () => {
    return Boolean(
      filters.city || 
      (filters.auctionPrice?.min !== undefined && filters.auctionPrice?.max !== undefined) || 
      (filters.buyNow?.min !== undefined && filters.buyNow?.max !== undefined)
    );
  };
  ...
  return (
    <>
      ...
      <div ref={popupRef} className="fixed md:absolute md:top-full left-0 right-0 md:left-auto 
        ...
        {/* მომენტალურად შესყიდვის ფასი */}
        <div className="mb-4">
          <label className="block mb-2">მომენტალურად შესყიდვის ფასი</label>
          <div className="flex flex-wrap gap-2">
            {[
              { min: 0, max: 100, label: '0 - 100' },
              { min: 100, max: 300, label: '100 - 300' },
              ...
              <button
                key={label}
                className={`px-4 py-2 rounded-full border transition-colors ${
                  filters.buyNow?.min === min && filters.buyNow?.max === max
                    ? 'bg-black text-white border-black'
                    : 'bg-white text-black border-gray-200 hover:border-black'
                }`}
                onClick={() => handlePriceRangeChange('buyNow', { min, max })}
              >
                {label}
              </button>
            ))}
